 ...
 */
public class FreewayH implements Drawable {
  ...
  public int maximumTruckVelocity;
  ...
  public void initialize(LatticeFrame spaceTime) {
    ...
    maximumTruckVelocity = 1 + maximumVelocity / 2;
    int d = roadLength/numberOfCars;
    x[0] = 0;
    v[0] = maximumTruckVelocity;
  ...
  public void step() {
    for(int i = 0;i<numberOfCars;i++) {
      xtemp[i] = x[i]%roadLength;
    }
    for(int i = 0;i<numberOfCars;i++) {
      if(i == numberOfCars%5 && v[i]<maximumTruckVelocity) {
        v[i]++;                                   // acceleration
      }
      else if (v[i]<maximumVelocity)
        v[i]++;
      ...
      if(i > 0 && x[i] > roadLength)
        for(int j = 0;j<numberOfCars;j++) {
          if(x[j] == x[i]-roadLength)
            break;
          if(j == numberOfCars-1)
            x[i] -= roadLength;
        }
      ...
      if (x[i] >= roadLength)
        x[i] = roadLength + (xtemp[i]+v[i])%roadLength;
      else
  ...
  public void computeSpaceTimeDiagram() {
    ...
    if(t<scrollTime) {
      for(int i = 0;i<numberOfCars;i++) {
        spaceTime.setValue(x[i]%roadLength, t, 1);
      }
    } else {                                       // scroll diagram
      ...
      for(int i = 0;i<numberOfCars;i++) {
        spaceTime.setValue(x[i]%roadLength, scrollTime-1, 1); // add new row
      }
    }
  }
  ...
  public void draw(DrawingPanel panel, Graphics g) {
    ...
    for(int i = 0;i<numberOfCars;i++) {
      if (x[i] < roadLength)
        ...
        road.setValue(x[i] - roadLength, 1, (byte) 1);
    }
    road.draw(panel, g);
